      ...
      <div className="my-5">
          {!allCategories || !allProducts  || !allBrands? <LodingScrean /> : <div className='container-fluied px-3 my-5 py-5'>
              <div className="row mt-3">
                <Slider {...settings2}>
                    {allProducts.map((pro, index) => pro.priceAfterDiscount ? 
                        <div key={index} className="product position-relative overflow-hidden">
                                <figure style={{height:'80vh'}} className='overflow-hidden position-relative'>
                                    <img className='w-100 proImg' src={pro.imageCover} alt={pro.title} />
                                    <figcaption className='py-2 position-absolute top-0 bottom-0 start-0 end-0 d-flex align-items-center align-content-center px-5 flex-wrap bg-black bg-opacity-75'>
                                        ...
                                        <div className='w-100'><Link to={`/product-detailes/${pro.id}`} className='text-decoration-none'><button className='btn btn-outline-light' title='detailes'> View more about this product <i className='fa fa-arrow-right'></i></button></Link></div>
                                    </figcaption>
                                </figure>
                        </div>
                    : '')}
              ...
              <div className="row gx-2 px-5 my-5 gy-2">
                ...
                <Slider {...settings}>
                    {allProducts.map((pro, index) => pro.ratingsAverage >= 4.5 ? <div id='homeTop' key={index} className="col-6  position-relative producInWideScreen text-white col-sm-4 col-md-3">
